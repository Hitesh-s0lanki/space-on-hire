...
const TaskTagsEditableField = ({ form, name, options }: Props) => {
                    ...
                    <FormControl>
                        {disabled ? <div
                            ...
                            {field.value && field.value.length !== 0 ? field.value.map((e: string, index: any) => (
                                <StyleTag key={index} text={e} />
                            )) : <span className="text-muted-foreground">Empty</span>}

                        </div> : (
                            <div className="w-full">
                                <CreatableSelect
                                    value={(field.value || []).map((e: string) => ({ label: e, value: e }))}
                                    options={options}
                                    placeholder=""
                                    onChange={(value: any) => {
                                        field.onChange(
...
export default TaskTagsEditableField;
